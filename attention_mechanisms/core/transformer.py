...
class TransformerBlock(nn.Module):
    ...
    def __call__(self,
                 ...
                 memory_states: Optional[Float[Array,
                                               "batch mem_len hidden"]] = None,
                 **kwargs) -> Union[
        ...
        if self.autoregressive:
            ...
            if self.attn_type == "mqa":
                attn_output, * \
                    cache_values = self.attn(
                        attn_input, memory, mask=mask, **kwargs)
            elif self.attn_type == "mla":
                attn_output, * \
                    cache_values = self.attn(attn_input, mask=mask, **kwargs)
            else:  # mha, gqa
                attn_output, * \
                    cache_values = self.attn(attn_input, mask=mask, **kwargs)
    ...
    def __call__(self,
        ...
        if self.autoregressive:
            ...
            for i, layer in enumerate(self.layers):
                ...
                cache_dict.update(
                    {f"{layer_prefix}_{k}": v for k, v in layer_cache.items()})
